#!/usr/bin/env python3
"""Live Pico XR-Toolkit to vt_human_v2 MuJoCo viewer.

This uses the new position-based pico_xrt conversion path instead of the older
xrobot quaternion retargeting path.
"""
...
from general_motion_retargeting.utils.pico_xrt import body_data_to_pico_xrt_frame
...
def main() -> None:
    ...
    print("[live-viewer] initializing XRobotStreamer")
    streamer = XRobotStreamer()
    print("[live-viewer] initializing GMR pico_xrt -> vt_human_v2")
    retargeter = GMR(
        src_human="pico_xrt",
        tgt_robot=args.robot,
        actual_human_height=args.actual_human_height,
    )
    print("[live-viewer] opening MuJoCo viewer")
    ...
    try:
        while viewer.viewer.is_running():
            ...
            if body_data is None:
                ...
                if now - last_log >= 2.0:
                    ...
                    print(
                        f"[live-viewer] waiting body_data no_body={no_body} "
                    ...
                    json.dumps(
                        {
                            ...
                            "actual_human_height": args.actual_human_height,
                            "body_data": body_data,
                            "qpos_pico_xrt_wxyz": qpos.tolist(),
                        }
                    )
                    + "\n"
                )
            ...
            if frames == 1 or now - last_log >= 2.0:
                dof = qpos[7:]
                print(
                    f"[live-viewer] frames={frames} root={np.round(qpos[:3], 4).tolist()} "
        ...
        if record_file is not None:
            print(f"[live-viewer] closed recording after frames={frames}")
            record_file.close()
        viewer.close()
